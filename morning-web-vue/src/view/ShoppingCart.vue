      ...
      <div class="add-info-box">
        <div class="add-info-detail">
          ...
          <div class="add-info-box-row">
            <div class="add-info-img">
              <img :src="newShoppingCart.package.picImg" alt="">
            </div>
            <div class="add-info-intro">
              <p>{{newShoppingCart.name}} {{newShoppingCart.package.kindName}}...</p>
              <p class="add-info-intro-detail">颜色：{{newShoppingCart.package.kindName}}/ 数量：{{newShoppingCart.count}}</p>
            </div>
          </div>
        </div>
        <div class="car-btn-group">
          ...
          <div class="car-btn-row">
            <router-link :to="{path:'/goodsDetail',query:{productId:newShoppingCart.productId}}">
      ...
      <div class="other-user-buy-row" v-for="(items,index1) in recommend" :key="index1">
        <div class="other-user-buy-item-box" v-for="(item,index2) in items" :key="index2">
          <div class="other-user-buy-item-img">
            <a href="item_detail.html"><img :src="item.picImg" alt=""></a>
          </div>
...
import {recommendListApi} from '../vuex/api';
...
export default {
  ...
  created () {
    recommendListApi('').then(res => {
      // console.log('查询商品推荐列表数据' + JSON.stringify(res));
      this.recommend = res;
    });
  },
  computed: {
