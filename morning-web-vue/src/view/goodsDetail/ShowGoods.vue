    ...
    <div class="item-detail-show">
      <div class="item-detail-left">
        <div class="item-detail-big-img">
          <img :src="goodsInfo.smallImg[imgIndex]" alt="">
        </div>
        <div class="item-detail-img-row">
          <div class="item-detail-img-small" v-for="(item, index) in goodsInfo.smallImg" :key="index" @mouseover="showBigImg(index)">
            <img :src="item" alt="">
          </div>
        </div>
      </div>
      <div class="item-detail-right">
        <div class="item-detail-title">
          <p>
            <span class="item-detail-express">校园配送</span> {{goodsInfo.name}}</p>
        </div>
        ...
        <div class="item-detail-price-row">
          <div class="item-price-left">
            <div class="item-price-row">
              <p>
                ...
                <span class="item-price">￥{{goodsInfo.showPrice}}</span>
              </p>
            </div>
            <div class="item-price-row">
              <p>
          ...
          <div class="item-price-right">
            <div class="item-remarks-sum">
              <p>累计评价</p>
              <p>
                <span class="item-remarks-num">{{goodsInfo.commentNumber}} 条</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="item-select">
          ...
          <div class="item-select-column">
            <div class="item-select-row" v-for="(items, index) in goodsInfo.kindList" :key="index">
              <div class="item-select-box" v-for="(item, index1) in items" :key="index1"
                   @click="select(index, index1)" :class="{'item-select-box-active': ((index * 3) + index1) === selectBoxIndex}">
...
import {addShoppingCartApi} from '../../vuex/api';
...
export default {
  ...
  methods: {
    ...
    select (index1, index2) {
      ...
      this.showPrice = this.goodsInfo.kindList[index1][index2].showPrice;
    },
    showBigImg (index) {
      this.imgIndex = index;
    },
    addShoppingCartBtn () {
      ...
      const data = {
        ...
        count: this.count,
        package: this.goodsInfo.kindList[index1][index2]
      };
      console.log('加入购物车数据' + JSON.stringify(data));
      // 添加本地缓存
      this.addShoppingCart(data);
      // 向后台保存
      addShoppingCartApi(data).then(res => {
        console.log('添加购物车响应数据:' + JSON.stringify(res));
